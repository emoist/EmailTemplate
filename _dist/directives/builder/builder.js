...
angular.module('email.directives', [
    ...
    .factory('storage', ['utils', 'variables', '$http', 'store', 'jwtHelper', '$location', '$rootScope', function (utils, variables, $http, store, jwtHelper, $location, $rootScope) {
        ...
        self.opts = {
             ...
             */
            get: function (email_id) {
                return new Promise(function (resolve, reject) {
                    try {
                        if (email_id == 'create') {
                            var email = {
                                is_template: 0,
                                template: JSON.parse(JSON.stringify(defaultEmail))
                            };
                            resolve(email);
                        }
                        else {
     ...
     */
    .config(['$translateProvider', '$translatePartialLoaderProvider', '$sceDelegateProvider','$compileProvider', 'variables',
        function ($translateProvider, $translatePartialLoaderProvider, $sceDelegateProvider, $compileProvider, variables) {
    ...
    .directive('emailBuilder', ['variables', function(variables) {
        return {
            restrict: 'E',
